...
# Attach the policy to Load Balancer Controller role
...
resource "aws_ec2_tag" "public_subnet_cluster_tags" {
  for_each    = toset(data.aws_subnets.public.ids)
  resource_id = each.value
  key         = "kubernetes.io/cluster/super-mario-cluster"
  value       = "owned"
}
...
output "cluster_security_group_id" {
  ...
  value       = aws_eks_cluster.mario.vpc_config[0].cluster_security_group_id
}
...
output "cluster_certificate_authority_data" {
  ...
  value       = aws_eks_cluster.mario.certificate_authority[0].data
}
